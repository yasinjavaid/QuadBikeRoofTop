...
public class FlipControl extends MonoBehaviour
{
	...
	function ApplyStopFlip()
	{
		var stopFlipFactor : Vector3 = Vector3.zero;
	...
	{
		var actualForwardDot : float = vp.forwardDot;
		var actualRightDot : float = vp.rightDot;
		var actualUpDot : float = vp.upDot;

		if (groundCheckDistance > 0)
		{
			var groundHit : RaycastHit;

			if (Physics.Raycast(tr.position, (-GlobalControl.worldUpDir + rb.velocity).normalized, groundHit, groundCheckDistance, GlobalControl.groundMaskStatic))
			{
