...
public class FollowAI : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		...
		//	Debug.Log (reverseTime == 0 && brakeTime == 0 && !(close && vp.localVelocity.z > 0.1f) ? (lookDot < 0.5f && lookDot > 0 && vp.localVelocity.z > 10 ? 0.5f - lookDot : 0) : (brakeTime > 0 ? brakeTime * 0.2f : 1));
			vp.SetBrake(reverseTime == 0 && brakeTime == 0 && !(close && vp.localVelocity.z > 0.1f) ? (lookDot < 0.5f && lookDot > 0 && vp.localVelocity.z > 10 ? 0.3f - lookDot : 0) : (brakeTime > 0 ? brakeTime * 0.1f : 1));
			if(quadBike==null)
			{
				quadBike=vp.transform.root.GetComponentInChildren<QuadBikeController>();
			}
			float value=reverseTime == 0 ? Mathf.Abs(Mathf.Pow(steerDot, (tr.position - target.position).sqrMagnitude > 20 ? 1 : 2)) * Mathf.Sign(steerDot) : -Mathf.Sign(steerDot) * (close ? 0 : 1);
			if(value<-0.2f){
				if(quadBike)
			...
			//Debug.Log ("Steering Value :"+value);
			vp.SetSteer(value);
