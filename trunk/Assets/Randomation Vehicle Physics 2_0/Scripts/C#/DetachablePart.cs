...
public class DetachablePart : MonoBehaviour
{
	...
	public bool isNotattach=false;
	void Start()
	{
		tr = transform;
		if (this.gameObject.transform.root.tag == "Player2")
			breakForce = breakForce * Constants.PlayerCarHealth [UserPrefs.currentEpisode - 1, UserPrefs.currentLevel - 1];
		else
			breakForce = breakForce * Constants.EnemyCarsHealth [UserPrefs.currentEpisode - 1, UserPrefs.currentLevel - 1];
		if (tr.parent)
		{
