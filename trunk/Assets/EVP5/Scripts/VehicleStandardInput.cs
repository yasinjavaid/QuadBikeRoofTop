...
//		float handbrakeInput = Mathf.Clamp01(Input.GetAxis(handbrakeAxis));
		...
		float steerInput = Mathf.Clamp(HudListener.steerPressedTime, -1.0f, 1.0f);
		float forwardInput = Mathf.Clamp01(!HudListener.isReverse ? HudListener.accelPressedTime : 0);
		float reverseInput = Mathf.Clamp01(HudListener.isReverse ? HudListener.accelPressedTime : 0);
		float handbrakeInput = Mathf.Clamp01(HudListener.brakePressedTime);
