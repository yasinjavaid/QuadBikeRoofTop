...
static NSString *const kPHRequestParameterLocationLongitude = @"lon";
static NSString *const kPHRequestParameterLocationLatitude = @"lat";
static NSString *const kPHRequestParameterLocationAccuracy = @"acc";
...
+ (id)requestForApp:(NSString *)token secret:(NSString *)secret placement:(NSString *)placement delegate:(id)delegate
{
    ...
    if (!!request) {
        ...
        return [[[self class] alloc] initWithApp:token secret:secret placement:placement delegate:delegate];
    }
}
            ...
            messageID:(NSString *)messageID campaignID:(NSString *)campaignID
{
    return [[[self class] alloc] initWithApp:token secret:secret contentUnitID:contentID messageID:
                messageID campaignID:campaignID];
}
...
- (NSDictionary *)additionalParameters
{
    ...
    [theAdditionalParameters setObject:[PHPreloader cacheQueryString] forKey:@"preloaded_creative_ids"];
    ...
    NSString *latitude = @"";
    NSString *accuracy = @"";
    if (location && [PHAdRequest locationOptOutStatus] == NO) {
        longitude = [NSString stringWithFormat:@"%f", location.coordinate.longitude];
        latitude = [NSString stringWithFormat:@"%f", location.coordinate.latitude];
        accuracy = [NSString stringWithFormat:@"%i", (int)floorf(location.horizontalAccuracy)];
    }
    
    theAdditionalParameters[kPHRequestParameterLocationLongitude] = longitude;
    theAdditionalParameters[kPHRequestParameterLocationLatitude] = latitude;
    theAdditionalParameters[kPHRequestParameterLocationAccuracy] = accuracy;
...
- (void)didSucceedWithResponse:(NSDictionary *)responseData
{
    ...
    if (PHGetShouldCacheCreative() && !_content.videoOverlayURL) {
...
- (void)send
{
    ...
    if (PHBaseURLIsSet()) {
        ...
        [[PHPublisherOpenRequest requestForApp:self.token secret:self.secret delegate:self] send];
    }
}
...
- (void)request:(PHAPIRequest *)request didFailWithError:(NSError *)error {
    if ([request isKindOfClass:[PHPublisherOpenRequest class]]) {
        // open failed, so we have to set the base URL with the default and resend the content request
        NSLog(@"Automatic open request failed: %@", error);
        PHSetBaseURL(PHGetBaseURL());
        _state = PHPublisherContentRequestInitialized;
        [self continueLoadingIfNeeded];
    } else if ([request isKindOfClass:[PHPublisherSubContentRequest class]]) {
        [self publisherSubContentRequest:(PHPublisherSubContentRequest *)request didFailWithError:error];
    }
}
...
- (void)publisherSubContentRequest:(PHPublisherSubContentRequest *)request didFailWithError:(NSError *)error {
    NSDictionary *errorDict = [NSDictionary dictionaryWithObjectsAndKeys:@"1", @"error", nil];
    [request.source sendCallback:request.callback withResponse:nil error:errorDict];
}
...
- (void)pushContent:(PHContent *)content
{
    ...
    [contentView redirectRequest:@"ph://rewards" toTarget:self action:@selector(requestRewards:callback:source:)];
    ...
    // NOTE: There should probably be a flag in the top level of the response to easily check if video
    if (!content.videoOverlayURL) {
        [self placeCloseButton];
    } else {
        [self hideCloseButton];
        
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(handleVideoTimeout:)
                                                     name:PH_VIDEO_TIMOUT_NOTIFICATION
                                                   object:nil];
    }
}
...
- (void)contentViewDidDismiss:(PHContentView *)contentView
{
    ...
    if ([self.contentViews count] == 0) {
        ...
        if ([(id<PHPublisherContentRequestDelegate>)self.delegate respondsToSelector:@selector(request:contentDidDismissWithType:)]) {
            PHPublisherContentDismissType *dismissType = (contentView.hasNoAd) ?
            PHPublisherNoContentTriggeredDismiss :
            PHPublisherContentUnitTriggeredDismiss;
            
            [(id<PHPublisherContentRequestDelegate>)self.delegate performSelector:@selector(request:contentDidDismissWithType:)
                                                                       withObject:self
                                                                       withObject:dismissType];
        } else if ([(id<PHPublisherContentRequestDelegate>)self.delegate respondsToSelector:@selector(requestContentDidDismiss:)]) {
            [(id<PHPublisherContentRequestDelegate>)self.delegate performSelector:@selector(requestContentDidDismiss:)
                                                                       withObject:self];
        }
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:PH_VIDEO_TIMOUT_NOTIFICATION object:nil];
}
...
- (void)handleVideoTimeout:(NSNotification *)notification {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:PH_VIDEO_TIMOUT_NOTIFICATION object:nil];
    [self placeCloseButton];
    [self showCloseButtonBecauseOfTimeout];
}
...
- (BOOL)isValidReward:(NSDictionary *)rewardData {
    ...
    NSString *idKey = [rewardData objectForKey:@"identifier_key"];
    
    NSDictionary *identifier =  @{idKey:[[[PHAdRequest identifiers] objectForKey:[rewardData objectForKey:@"identifier_key"]] lowercaseString]};
    NSString *nonce =       [rewardData objectForKey:@"nonce"];
    NSString *reward_name = [rewardData objectForKey:@"reward_name"];
    NSString *signature =   [rewardData objectForKey:@"signature"];
    
    if (nil == identifier || nil == nonce || nil == reward_name || nil == signature) {
        PH_DEBUG(@"Missed required field in the reward dictionary: %@", rewardData);
        return NO;
    }
    
    NSString *expectedSignature = [PHAdRequest v4SignatureWithIdentifiers:identifier
                                                                    token:reward_name
                                                                    nonce:nonce
                                                             signatureKey:self.secret];
    
    if ([expectedSignature isEqualToString:signature] && [[PHAPIRequest allNonces] containsObject:nonce]) {
        [[PHAPIRequest allNonces] removeObject:nonce];
        return YES;
    }
...
- (void)requestRewards:(NSDictionary *)queryParameters callback:(NSString *)callback source:(PHContentView *)source {
    ...
    if ([self isValidReward:queryParameters]) {
        PHReward *reward = [[PHReward alloc] init];
        reward.rewardName = [queryParameters objectForKey:@"reward_name"];
        reward.quantity = 1;
...
- (NSString *)serializedCustomDimensions
{
    ...
    if (0 < [self.customDimensions count])
    {
        if ([NSJSONSerialization isValidJSONObject:self.customDimensions])
        {
            ...
            NSData *theJSONData = self.customDimensions ? [NSJSONSerialization dataWithJSONObject:self.customDimensions
                                                                                          options:0 error:&theError] : nil;
